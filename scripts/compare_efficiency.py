...
def compare_metrics(results_dir: Path = None):
    ...
    print("=" * 80)
    print(
        f"{'Experiment':<30} {'Time(s)':<12} {'Steps':<8} {'GPU Mem(MB)':<15} {'Tokens/s':<12}"
    )
    print("-" * 80)
    ...
    if len(experiments) > 1:
        fastest = min(
            experiments, key=lambda x: x.get("unlearning_time_seconds", float("inf"))
        )
        most_memory_efficient = min(
            experiments, key=lambda x: x.get("peak_gpu_memory_mb", float("inf"))
        )
        ...
        print("\nBest Performers:")
        print(
            f"  Fastest: {fastest['experiment']} ({fastest.get('unlearning_time_seconds', 0):.2f}s)"
        )
        print(
            f"  Most Memory Efficient: {most_memory_efficient['experiment']} ({most_memory_efficient.get('peak_gpu_memory_mb', 0):.2f} MB)"
        )

        if has_positive_throughput:
            highest_throughput = max(
                experiments, key=lambda x: x.get("tokens_per_second", 0)
            )
            print(
                f"  Highest Throughput: {highest_throughput['experiment']} ({highest_throughput.get('tokens_per_second', 0):.2f} tokens/s)"
            )
        else:
            print(
                "  Highest Throughput: N/A (tokens_per_second unavailable or zero for all experiments)"
            )
        print()
